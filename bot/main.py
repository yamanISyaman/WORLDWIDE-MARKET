...
def shipping(shipping_query):
    ...
    shipping_options = [
        telebot.types.ShippingOption(id="pickup", title="Local Pickup").add_price(
            telebot.types.LabeledPrice("Pickup", 0)
        ),
        telebot.types.ShippingOption(id="delivery", title="Delivery").add_price(
            telebot.types.LabeledPrice("delivery", 500)
        ),
    ]
...
def checkout(pre_checkout_query):
    ...
    bot.answer_pre_checkout_query(
        ...
        ok=True,
        error_message="SEEMS LIKE YOU'RE TRYING TO BUY FROM AN EXPIRED INVOICE\n\nPLEASE START A NEW INVOICE AND USE IT INSTEAD",
    )
