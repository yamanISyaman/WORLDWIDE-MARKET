...
def check_for_newuser(message):
    ...
    if message.chat.id in users:
        ...
        sql(
            ...
            (message.chat.id, get_fullname(message)),
        )
        return True
...
def cancelling(message):
    ...
    responses.bot.send_message(
        chat_id=message.chat.id, text="--Process Is Cancelled--", reply_markup=markup
    )
    responses.buttons_input(message.chat.id, "/start")
    # deleting the cancelled data
    sql(
        "DELETE FROM products WHERE owner_id = ? AND photo_id IS NULL",
        (message.chat.id,),
    )
    return
...
def make_post(data: tuple):
    ...
    text = f"{name}\n\n{price} $USD\n\n{description}\n\n{country} || {city}"
    entities = [
        telebot.types.MessageEntity("bold", 0, len(name) + len(price) + 6),
        telebot.types.MessageEntity(
            "italic", text.find(country), len(country) + len(city)
        ),
    ]
    ...
    for admin in admins_sql:
        ...
        if text != None:
            if entities == None:
                ...
                responses.bot.send_message(
                    chat_id=admin[0], text=text, entities=entities
                )
    return admins
