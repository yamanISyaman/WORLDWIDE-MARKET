...
outputs = {"/start":{"buttons":['Find Product', 'Add Product', 'My Cart', 'My Products', 'Bought Products', 'My Balance', 'Report An Issue'], "text":"Main Menu: "}, "Find Product": {"buttons":["Show All Products", "Search", "Back To Menu"], "text": "you can see all the posted products or use the search"}}
...
def buttons_input(chat_id, message_text) :
    ...
    if message_text in outputs.keys():
        ...
        while i < len(buttons_list):
            try:
                markups = [telebot.types.KeyboardButton(j) for j in buttons_list[i : i+2]]
                markup.add(*markups)
            except:
    ...
    # adding the button of cancelling a process
        ...
        markup.add("Cancel!")
        msg = bot.send_message(chat_id=chat_id, text="You Can Cancel this Process By Writing 'Cancel!' or Using The Below Cancel Button" , reply_markup=markup)
        return
    else:
        return
...
def process_product_name(message):
    ...
    if len(message.text) > 30 :
        msg = bot.send_message(message.chat.id, "that's too long for a name, please change it to a brief one:")
        bot.register_next_step_handler(msg, process_product_name)

    else:
        product_name = message.text
        sql('INSERT INTO products(name, owner_id) VALUES (?, ?)', ( product_name, message.chat.id))
        msg = bot.send_message(message.chat.id, "Great! now add the price in $USD currency\n\nNOTE:\nWhen someone buy your product we cut 2.5$ from the price as fees and the rest is added to your balance")
...
def process_product_price(message):
    ...
    # if not a number, then ask to re-enter 
        ...
        msg = bot.send_message(message.chat.id, "OH a price must be number, please re-enter a valid price number:")
        bot.register_next_step_handler(msg, process_product_price)
        return

    if product_price < 5:
        msg = bot.send_message(message.chat.id, "SORRY, The price can't be less than 5 dollars\nplease enter a price that's more than 5$")
        bot.register_next_step_handler(msg, process_product_price)
        return
    elif product_price > 10000:
        msg = bot.send_message(message.chat.id, "SORRY, The price can't be more than 10,000 dollars\nplease enter a price that's less than 10,000$")
        bot.register_next_step_handler(msg, process_product_price)
        return
            
    # adding price to database
    sql('UPDATE products SET price = ? WHERE owner_id = ? AND photo_id IS NULL', (product_price, message.chat.id))
    msg = bot.send_message(message.chat.id, "please now add a brief description that's between 30 and 120 letters:")
...
def process_product_desc(message):
    ...
    if len(desc) < 30:
        msg = bot.send_message(message.chat.id, "the description can't be less than 30 letters:")
        bot.register_next_step_handler(msg, process_product_desc)
        
    elif len(desc) > 120:
        msg = bot.send_message(message.chat.id, "the description can't be more than 120 letters:")
        bot.register_next_step_handler(msg, process_product_desc)

    else:
        # adding description to database
        sql('UPDATE products SET description = ? WHERE owner_id = ? AND photo_id IS NULL', (desc, message.chat.id))
...
def process_product_country(message):
    ...
    # reject a too long country name
        ...
        sql('UPDATE products SET country = ? WHERE owner_id = ? AND photo_id IS NULL', (country, message.chat.id))
...
def process_product_city(message):
    ...
    # reject a too long city name
        ...
        city = message.text
        # adding country to the database
        sql('UPDATE products SET city = ? WHERE owner_id = ? AND photo_id IS NULL', (city, message.chat.id))
        msg = bot.send_message(message.chat.id, "Now please send a photo that best describes your product:")
...
def pending_posts(message, next_product=0, my_message=0):
    ...
    pending_products = sql("SELECT id, name, price, description, country, city, photo_id FROM products WHERE is_approved = 0")
    ...
    if list_len == 1:
        ...
        inline_buttons = make_inlines(('Approve', f'A{id}'),('DisApprove', f'D{id}'), ("Next", f"N{nex}"))

    if my_message == 0:
        bot.delete_message(message.chat.id, message.id)
        bot.send_photo(chat_id=message.chat.id, photo=photo_id , caption=text, reply_markup=inline_buttons, caption_entities=entities)
    else:
        media = telebot.types.InputMediaPhoto(photo_id, caption=text)
        bot.edit_message_media(media, message.chat.id, message.id, reply_markup=inline_buttons)
...
def answercall(call):
    ...
    if identifier == "A":
        owner_id = sql("SELECT owner_id FROM products WHERE id = ?", (product_id,))
        bot.send_message(owner_id[0][0], f"Your post with the text:\n{text[:-40]}\nWAS APPROVED BY THE ADMINS")
    ...
    # handling the 'Remove' callback query in cart
        ...
        sql("DELETE FROM cart WHERE product_id = ? AND user_id = ?", (product_id, chat_id))
...
def process_prduct_showing(message):
    if message.text == "Show All Products":
        ...
        msg = bot.send_message(message.chat.id, "Ops, seems like an invalid input\nPlease choose one of the below buttons")
        bot.register_next_step_handler(msg, process_prduct_showing)
...
def show_all_products(message, next_product=0, my_message=0, removing=0):
    ...
    approved_products = sql("SELECT id, name, price, description, country, city, photo_id FROM products WHERE is_approved = 1")
    ...
    inlines2 = [('Add To Cart', f'C{id}'),('Buy', f'B{id}'), ("Previous", f"P{pre}"), ("Next", f"F{nex}")]
    ...
    if my_message == 0 or removing == 1:
        bot.delete_message(message.chat.id, message.id)
        msg = bot.send_photo(chat_id=message.chat.id, photo=photo_id , caption=f"{text}", reply_markup=inline_buttons, caption_entities=entities)
        return msg
    else:
        media = telebot.types.InputMediaPhoto(photo_id, caption=text)
        bot.edit_message_media(media, message.chat.id, message.id, reply_markup=inline_buttons)
...
def post_search(message, search_w=0, next_product=0, my_message=0):
    ...
    approved_products = sql("SELECT id, name, price, description, country, city, photo_id FROM products WHERE is_approved = 1 AND id IN (SELECT id FROM products WHERE description LIKE ? OR name LIKE ? OR city LIKE ? OR country LIKE ?)", (f"%{search_word}%", f"%{search_word}%", f"%{search_word}%", f"%{search_word}%"))
    list_len = len(approved_products)
    # if there's no posts, let the user know
    if list_len == 0:
    ...
    if list_len == 1:
        ...
        inline_buttons = make_inlines(('Add To Cart', f'C{id}'),('Buy', f'B{id}'), ("Previous", f"S{pre}"), ("Next", f"T{nex}"))
    if my_message == 0:
        bot.delete_message(message.chat.id, message.id)
        msg = bot.send_photo(chat_id=message.chat.id, photo=photo_id , caption=text, reply_markup=inline_buttons, caption_entities=entities)
        buttons_input(message.chat.id, "/start")
    else:
        media = telebot.types.InputMediaPhoto(photo_id, caption=text)
        bot.edit_message_media(media, message.chat.id, message.id, reply_markup=inline_buttons)
...
def make_payment(chat_id, product_id):
    # product infos
    data = sql("SELECT name, description, price From products WHERE id = ?", (product_id,))
    name = data[0][0]
    price = data[0][2]
    description = data[0][1]
    # make price
    prices = [telebot.types.LabeledPrice(label=name, amount=int(price*100)), telebot.types.LabeledPrice(label="fees", amount=int(250))]
    # sending the invoice
    bot.send_invoice(chat_id, name, description, f"{product_id}", Provider_Token, 'USD', prices, is_flexible=True)
...
def payment_action(message):
    ...
    sql("INSERT INTO history (user_id, product_id, charge_id) VALUES(?, ?, ?)", (chat_id, product_id, charg_id))
    sql("UPDATE users SET balance = balance + ? WHERE id = (select owner_id FROM products WHERE id = ?)", (((amount - 500) / 100), product_id))
    product_data = sql("SELECT owner_id, name FROM products WHERE id = ?",(product_id,))
    # notify user
    if option == "delivery":
        bot.send_message(chat_id,"Your product will be delivered within a week\n\nThanks for using our market")
    elif option == "pickup":
        bot.send_message(chat_id, "Thanks for using our market\n\nThe product will be in our center that resides in your city within 5 days")
    bot.send_message(chat_id, "YOU SHOULD GIVE THE FOLLOWING CODE TO OUR EMPLOYEES TO RECEIVE YOUR PRODUCT\nSO PLEASE KEEP THE CODE IN A SAFE PLACE\n\n")
    entities = [telebot.types.MessageEntity("spoiler", 0, len(charg_id)), telebot.types.MessageEntity("code", 0, len(charg_id))]
    bot.send_message(chat_id, charg_id, entities = entities)
    # tell the product owner
    bot.send_message(product_data[0][0], f"HEY, SOMEONE HAS JUST BOUGHT {product_data[0][1]} FROM YOU, AND THE AMOUNT IS ADDED TO YOUR BALANCE")
    tell_admins(f"THE USER {product_data[0][0]} HAS JUST BOUGHT {product_data[0][1]} FROM {chat_id} ")
...
def show_cart(message, next_product=0, my_message=0, removing=0):
    ...
    cart_products = sql("SELECT id, name, price, description, country, city, photo_id FROM products WHERE id in (SELECT product_id FROM cart WHERE user_id = ?)", (message.chat.id,))
    ...
    if list_len == 1:
        ...
        inline_buttons = make_inlines(('Buy', f'B{id}'), ('Remove', f'R{id}'), ("Previous", f"Z{pre}"), ("Next", f"X{nex}"))
...
def add_to_cart(chat_id, product_id, call):
    ...
    user_cart = sql("SELECT user_id, product_id FROM cart WHERE user_id = ?", (chat_id,))
    if (chat_id, product_id) in user_cart:
        bot.send_message(chat_id, "PRODUCT IS ALREADY IN YOUR CART")
    else:
        sql("INSERT INTO cart (user_id, product_id) VALUES (?, ?)", (chat_id, product_id))
...
def show_user_products(message, next_product=0, my_message=0):
    ...
    user_products = sql("SELECT id, name, price, description, country, city, photo_id FROM products WHERE is_approved = 1 AND owner_id = ?", (message.chat.id,))
...
def show_history(message, next_product=0, my_message=0, removing=0):
    ...
    history_data = sql("SELECT product_id , id, done FROM history WHERE user_id = ?", (message.chat.id,))
    ...
    # if the history is empty
        ...
        bought_products = sql("SELECT id, name, price, description, country, city, photo_id FROM products WHERE id = ?", (history_data[index][0],))
    ...
    if list_len == 1:
        ...
        if is_done == 1:
            inline_buttons = make_inlines(("Previous", f"I{pre}"), ("Next", f"O{nex}"), ('Delete', f'H{id}'))
        else:
...
def send_nf_process(message):
    ...
    try:
        ...
        msg = bot.send_message(chat_id, "USER ID MUST BE A NUMBER, PLEASE ENTER A VALID ID")
        bot.register_next_step_handler(msg, send_nf_process)
        return
...
def no_owner_action(message):
    ...
    msg = bot.send_message(message.chat.id, "THIS BOT IS NOT WORKING, BECAUSE THE OWNERS HAVEN'T VALIDATE THEMSELVES YET\n\n""IF YOU'RE THE OWNER THEN PLEASE ADD THE RIGHT PASSWORD TO START THE BOT")
...
def process_owner_password(message):
    ...
    if no_owner():
        if message.text == Owner_Password:
            ...
            sql("INSERT INTO users (id, name, is_admin) VALUES(?, ?, ?) ",(chat_id, name, 2))
            bot.send_message(chat_id, "YOU'RE NOW OFFICIALLY THE OWNER")
            buttons_input(chat_id, "/start")

        else:
            msg = bot.send_message(chat_id, "SEEMS LIKE YOU'RE NOT THE OWNER, PLEASE RESTART THE BOT WHEN THE OWNERS VALIDATE THEMSELVES, OR RE-ENTER THE PASSWORD:")
            bot.register_next_step_handler(msg, process_owner_password)

    else:
        bot.send_message(chat_id, "THE OWNERS ARE ALREADY VALIDATED, HAVE A NICE TIME USING OUR MARKET :)")
